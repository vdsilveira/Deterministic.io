...
(async () => {
  ...
  for (const addr of addresses) {
    console.log('   -', addr.substring(0, 45) + '...');
  }
  
  const wif = await page.locator('.wif-value').textContent().catch(() => 'Não gerado');
  console.log('🔑 WIF:', wif.substring(0, 35) + '...');
  
  console.log('\n✅ Site testado! O browser continuará aberto.');
  console.log('💡 Você pode interagir com o site. Feche o browser quando terminar.');
  
  // NÃO fecha o browser - mantém aberto para interação
  console.log('\n⏳ Browser aberto. Pressione Ctrl+C no terminal para sair...');
